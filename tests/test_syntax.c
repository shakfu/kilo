...
TEST(syntax_python_comment) {
     ...
     * Python is configured with "#" (one char), but the code checks both
     * scs[0] and scs[1], so it only matches if '#' is followed by '\0'.
     * This test documents the current (incorrect) behavior. */
    for (int i = 0; i < row.rsize; i++) {
        ASSERT_EQ(row.hl[i], HL_NORMAL);  /* Currently NOT highlighted */
    }
